    ...
    <LoadScript googleMapsApiKey={apiKey}>
      ...
      {selectedPlace &&
        <div className={styles.selectedPlace}>
          ...
          <a className={styles.googleMapsLink} target="_blank" href={`https://maps.google.com/maps?q=${selectedPlace.name}&ll=${selectedPlace.location.coordinate.latitude},${selectedPlace.location.coordinate.longitude}`}>
            <Image src={GoogleMaps} alt={'Google Maps'} width={40} height={40} />
          </a>
          <button className={styles.closeButton} onClick={() => setSelectedPlace(null)}><X size={32} /></button>
        </div>
      }

      {!showsAll && <button className={styles.showAllButton} onClick={() => router.push("/map")}>Show all</button>}
    </LoadScript>
  );
};
